...
import { Film, Home, Layers, User, Bell, Settings, Tv, BookOpen, Radio, Menu, X } from "lucide-react"
...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-800/40 bg-slate-950/80 backdrop-blur-xl shadow-[0_12px_60px_rgba(8,47,73,0.45)]">
        ...
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              key={href}
              href={href as any}
              className={`relative inline-flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 2xl:text-base 2xl:px-5 2xl:py-2.5 focus-ring ${isActive
                ? 'text-white'
            ...
            >
              ...
              {isActive && <span className="absolute bottom-0 left-3 right-3 h-0.5 rounded-full bg-gradient-to-r from-cyan-400 to-blue-500 nav-active-line" />}
            </Link>
          )
        })}
      </nav>
            ...
            {navItems.map(({ href, label, icon: Icon }) => {
              ...
              return (
                <Link
                  ...
                  className={`inline-flex items-center gap-3 rounded-xl px-4 py-4 text-base font-medium transition active:scale-95 ${isActive
      ...
      {!mobileMenuOpen && (
        <nav className="mx-auto flex w-full max-w-7xl gap-3 overflow-x-auto px-4 pb-3 pt-1 text-sm lg:hidden scrollbar-hide snap-x">
          {navItems.map(({ href, label, icon: Icon }) => {
            const isActive = pathname === href || (href !== "/" && pathname.startsWith(href))
            const isLive = href === "/live"
            return (
              <Link
                key={href}
                href={href as any}
                className={`inline-flex flex-shrink-0 items-center gap-2 rounded-full border px-5 py-2.5 transition snap-start focus:outline-none focus:ring-2 focus:ring-cyan-400 ${isActive
                  ? 'border-cyan-400/60 bg-cyan-500/10 text-cyan-100 shadow-[0_0_15px_rgba(6,182,212,0.2)]'
